...
class Camera:
    ...
    def clip_in_cam(self, sprite: Engine.Sprite.Sprite):
        if sprite.source_rect_pre is None:
            sprite.source_rect = pg.Rect(0, 0, sprite.world_rect.w, sprite.world_rect.h)
        else:
    ...
    def draw_line(self, line_x1, line_y1, line_x2, line_y2, color=pg.Color(255, 255, 255), is_world=True):
        ...
        x1_check = line_x1 < self.display_port.x or line_x1 > self.display_port.x + self.display_port.w
        y1_check = line_y1 < self.display_port.y or line_y1 > self.display_port.y + self.display_port.h
        x2_check = line_x2 < self.display_port.x or line_x2 > self.display_port.x + self.display_port.w
        y2_check = line_y2 < self.display_port.y or line_y2 > self.display_port.y + self.display_port.h
