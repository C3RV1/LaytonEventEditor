...
class EventPlayer(TwoScreenRenderer.TwoScreenRenderer):
    def __init__(self, event_id):
        ...
        self.top_bg = ScreenShaker.ScreenShaker(self.top_screen_group)
        self.top_screen_group.change_layer(self.top_bg, -1000)
        self.top_bg.post_shake = lambda: self.run_gds_command()

        self.bottom_bg = ScreenShaker.ScreenShaker(self.bottom_screen_group)
        self.bottom_screen_group.change_layer(self.bottom_bg, -1000)
        self.bottom_bg.post_shake = lambda: self.run_gds_command()

        self.top_fader = ScreenFader.ScreenFader(self.top_screen_group)
        ...
        self.bottom_fader = ScreenFader.ScreenFader(self.bottom_screen_group)
        ...
        self.back_translucent = Engine.Sprite.Sprite(self.bottom_screen_group)
        self.bottom_screen_group.change_layer(self.back_translucent, -100)
    ...
    def run_gds_command(self):
        while True:
            ...
            if next_command.command == 0x2:
                ...
                Debug.log("Starting dialogue", self)
...
def test_event(event_id):
    ...
    event_player = EventPlayer(event_id)
    event_player.load()
    while event_player.running:
        event_player.run()
